...
**解释**：每个 bug 修复必须伴随回归测试。测试不仅要验证修复有效，还要确保同样的 bug 不会在未来代码改动中再次出现。

**具体表现**：
- 修复 bug 前，先写测试复现问题（测试应该失败）
- 修复后，测试应该通过
- 测试命名明确：`test_operator_add_struct_field_order` 而非 `test_fix`
- 测试注释说明 bug 背景、修复日期、关键行为

**例外**：纯粹的重构（不改变行为）或文档修复不需要回归测试。

**为什么**：
- 个人项目代码频繁改动，没有回归测试很容易"修复一个 bug，引入另一个"
- 测试是代码意图的可执行文档
- 6 个月后回看代码，测试告诉你为什么这样写
...
fn test_operator_add_struct_field_order() {
    ...
    // Bug: call_operator_closure 中的 BuildStruct 多了 reverse()
...
- 修改一行代码到看到效果 < 1秒（Phase 5 热重载目标）
- 热重载不丢失运行状态（Phase 5 目标）
- 语法演进时，旧代码迁移成本 < 10分钟

## 当前阶段目标（Phase 3）

- 核心语言功能完整（语法、类型、运算符重载）
- 字节码解释器稳定运行
- 测试覆盖率 > 80%
- 为 JIT 和 热重载 打下架构基础
