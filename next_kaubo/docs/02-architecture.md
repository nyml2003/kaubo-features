# Kaubo 项目架构

> ⚠️ **注意：本文档包含已完成计划和目标架构的描述**
>
> 部分章节（如 Lexer 改造 Phase 0）已完成，部分（如 Phase 1+）仍在计划中。
> 
> 当前实际代码状态请参考 `AGENTS.md`。

> 整体架构、核心组件、日志系统与配置管理

## 目录

1. [整体架构](#1-整体架构)
2. [分层设计](#2-分层设计)
3. [日志系统](#3-日志系统)
4. [配置管理](#4-配置管理)
5. [核心组件](#5-核心组件)
6. [内存布局](#6-内存布局)
7. [文件结构](#7-文件结构)

---

## 1. 整体架构

```
┌─────────────────────────────────────────────────────────────┐
│                        应用层 (Application)                   │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │    CLI       │  │   静态库API   │  │   测试框架    │      │
│  │  (main.rs)   │  │  (lib.rs)    │  │  (tests/)    │      │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘      │
└─────────┼─────────────────┼─────────────────┼───────────────┘
          │                 │                 │
          └─────────────────┼─────────────────┘
                            │
┌───────────────────────────▼─────────────────────────────────┐
│                      编排层 (Orchestration)                   │
│                        src/api.rs                            │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  pub fn compile_and_run(source: &str) -> Result<...>  │  │
│  │  pub fn compile(source: &str) -> Result<...>          │  │
│  │  pub fn execute(chunk: &Chunk) -> Result<...>         │  │
│  └──────────────────────────────────────────────────────┘  │
└───────────────────────────┬─────────────────────────────────┘
                            │
        ┌───────────────────┼───────────────────┐
        ▼                   ▼                   ▼
┌───────────────┐  ┌───────────────┐  ┌───────────────┐
│   编译阶段     │  │   执行阶段     │  │   基础设施     │
├───────────────┤  ├───────────────┤  ├───────────────┤
│ • Lexer       │  │ • VM          │  │ • Config      │
│ • Parser      │  │ • Memory      │  │ • Logger      │
│ • Compiler    │  │ • Stdlib      │  │ • Error       │
└───────────────┘  └───────────────┘  └───────────────┘
```

---

## 2. 分层设计

### 2.1 API 分层

| 层级 | 模块 | 职责 | 使用场景 |
|------|------|------|---------|
| **应用层** | `main.rs` | CLI 参数解析、流程调度 | 命令行工具 |
| **编排层** | `api.rs` | 阶段组合、错误处理 | 库用户、测试 |
| **阶段层** | `lexer/parser/compiler/vm` | 独立功能实现 | 单独调用、调试 |
| **基础设施** | `config/logger` | 全局配置、日志 | 贯穿所有层 |

### 2.2 调用示例

```rust
// 应用层 (CLI)
fn main() {
    let cli = Cli::parse();
    kaubo::config::init(build_config(&cli));
    kaubo::logger::init();
    
    let result = kaubo::compile_and_run(&source)?;
    println!("{}", result.value);
}

// 编排层 (API)
pub fn compile_and_run(source: &str) -> Result<ExecuteOutput> {
    let tokens = lex(source)?;      // 阶段1
    let ast = parse(&tokens)?;       // 阶段2
    let compiled = compile(&ast)?;   // 阶段3
    execute(&compiled)               // 阶段4
}

// 阶段层 (独立使用)
let tokens = kaubo::lex(source)?;
```

---

## 3. 日志系统

### 3.1 设计目标

- **分阶段控制**: 可独立控制 lexer/parser/compiler/vm 的日志级别
- **结构化输出**: 支持 JSON 格式供工具解析
- **低开销**: 未启用的日志在编译期优化掉
- **跨度追踪**: 支持嵌套调用链追踪

### 3.2 日志分级

| 级别 | 用途 | 示例 |
|------|------|------|
| `TRACE` | 最详细，逐行调试 | 每个字符处理、每条指令执行 |
| `DEBUG` | 开发调试 | Token生成、AST节点、函数调用 |
| `INFO` | 正常流程信息 | 阶段开始/完成、编译成功 |
| `WARN` | 警告 | 不建议的用法、性能问题 |
| `ERROR` | 错误 | 编译错误、运行时错误 |

### 3.3 阶段过滤

```rust
// 只查看 Parser 的 DEBUG 以上日志
$ kaubo script.kaubo --log-parser=debug

// 查看全部 TRACE，但 VM 只显示 INFO
$ kaubo script.kaubo -vvv --log-vm=info
```

### 3.4 日志格式

**Pretty 格式**（开发使用）:
```
2024-02-10T10:23:45.123Z DEBUG kaubo::parser: parse variable_declaration
2024-02-10T10:23:45.124Z DEBUG kaubo::compiler: compile binary_op Add
2024-02-10T10:23:45.125Z TRACE kaubo::vm: op=LoadConst0 stack=[]
2024-02-10T10:23:45.126Z TRACE kaubo::vm: op=LoadConst1 stack=[SMI(1)]
```

**JSON 格式**（工具集成）:
```json
{
  "timestamp": "2024-02-10T10:23:45.123Z",
  "level": "DEBUG",
  "target": "kaubo::parser",
  "fields": {
    "message": "parse variable_declaration",
    "line": 10
  }
}
```

### 3.5 代码中使用

```rust
use tracing::{debug, error, info, instrument, trace, warn};

// 简单日志
debug!("Processing {} tokens", count);

// 结构化日志
debug!(token.kind = ?kind, token.value = %value, "Produced token");

// 函数级跨度（自动记录进入/退出）
#[instrument(target = "kaubo::lexer", skip(source))]
pub fn lex(source: &str) -> Result<Vec<Token>> {
    info!("Starting lexer");
    // ...
}

// 条件日志
if tracing::enabled!(tracing::Level::TRACE) {
    trace!("Expensive debug: {:?}", expensive_operation());
}
```

---

## 4. 配置管理

### 4.1 全局配置单例

```rust
// src/config.rs
use once_cell::sync::OnceCell;

static GLOBAL_CONFIG: OnceCell<Config> = OnceCell::new();

pub fn init(config: Config) {
    GLOBAL_CONFIG.set(config).ok();
}

pub fn config() -> &'static Config {
    GLOBAL_CONFIG.get().expect("Config not initialized")
}
```

### 4.2 配置结构

```rust
pub struct Config {
    /// 日志配置
    pub log: LogConfig,
    /// 执行限制
    pub limits: LimitConfig,
    /// 编译选项
    pub compiler: CompilerConfig,
}

pub struct LogConfig {
    pub global: Level,           // 默认级别
    pub lexer: Option<Level>,    // None = 使用 global
    pub parser: Option<Level>,
    pub compiler: Option<Level>,
    pub vm: Option<Level>,
}

pub struct LimitConfig {
    pub max_stack_size: usize,      // 默认 1024
    pub max_recursion_depth: usize, // 默认 256
}

pub struct CompilerConfig {
    pub emit_debug_info: bool,  // 是否生成调试信息
}
```

### 4.3 CLI 配置映射

```bash
# 基础级别
kaubo script.kaubo              # WARN (默认)
kaubo script.kaubo -v           # INFO
kaubo script.kaubo -vv          # DEBUG
kaubo script.kaubo -vvv         # TRACE

# 阶段级控制
kaubo script.kaubo --log-lexer=trace
kaubo script.kaubo --log-parser=off --log-vm=debug
```

---

## 5. 编译器前端架构 (Lexer → Parser → AST)

### 5.0 架构概览

```
┌─────────────────────────────────────────────────────────────────┐
│                     Compiler Frontend                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Source ──► Lexer ──► Token Stream ──► Parser ──► AST            │
│   Text      (LAYER 1)   (Interface)    (LAYER 2)   (Tree)        │
│                                                                  │
│  ┌──────────┐      ┌──────────┐      ┌──────────────────────┐   │
│  │CharStream│      │  Token   │      │  Recursive Descent   │   │
│  │Scanner   │─────►│  (Kind+  │─────►│  + Pratt Parser      │   │
│  │(Mode)    │      │  Span)   │      │                      │   │
│  └──────────┘      └──────────┘      └──────────────────────┘   │
│                                                          │       │
│                                                          ▼       │
│                                               ┌──────────────────┤
│                                               │  Expr/Stmt/Module │
│                                               │  (AST Nodes)      │
│                                               └──────────────────┘
└─────────────────────────────────────────────────────────────────┘
```

**为什么方案主要关注 Lexer？**

Lexer 是整个编译器的基础层，其设计质量直接影响 Parser、LSP、IDE 等上层功能。当前的多状态机架构存在扩展性差、性能低、维护成本高等问题，必须先解决。Parser 采用经典的递归下降 + Pratt 表达式解析，架构健康，无需大规模改造，只需适配新的 Lexer 接口。

---

### 5.1 词法分析器 (Lexer)

**状态**: ✅ 已完成手写 Scanner 改造（原多状态机已移除）  
**位置**: `src/kit/lexer/`  
**日志目标**: `kaubo::lexer`

#### 文件结构

```
src/kit/lexer/
├── mod.rs              # 模块导出
├── types.rs            # Coordinate（供 Parser 使用）
├── core/
│   ├── position.rs     # SourcePosition + SourceSpan
│   └── stream.rs       # CharStream（字符流抽象）
├── scanner.rs          # Scanner trait + Token 结构
├── kaubo.rs            # KauboScanner 实现
└── lexer.rs            # Lexer 入口
```

#### 5.1.1 三层架构设计

```
┌─────────────────────────────────────────────────────────────────┐
│  LAYER 3: Language Frontends                                     │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐                           │
│  │ Kaubo   │ │  JSON   │ │Vue/HTML │   声明式语法定义            │
│  │         │ │         │ │Template │   快速语法迭代              │
│  └────┬────┘ └────┬────┘ └────┬────┘                           │
├───────┼───────────┼───────────┼─────────────────────────────────┤
│       │           │           │        LAYER 2: Core            │
│       ▼           ▼           ▼                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │              Universal Scanner Engine                    │   │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐    │   │
│  │  │ Scanner  │ │ Token    │ │ Error    │ │ Mode     │    │   │
│  │  │ Trait    │ │ Builder  │ │ Recovery │ │ System   │    │   │
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘    │   │
│  └─────────────────────────────────────────────────────────┘   │
├─────────────────────────────────────────────────────────────────┤
│  LAYER 1: Infrastructure                                         │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐           │
│  │CharStream│ │Position  │ │ RingBuf  │ │Encoding  │           │
│  │(UTF-8/16)│ │Tracker   │ │(流式)    │ │(BOM/CRLF)│           │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘           │
└─────────────────────────────────────────────────────────────────┘
```

#### 5.1.2 Lexer-Parser 接口契约

```rust
/// Token 结构（Parser 消费的最小单元）
pub struct Token<K> {
    pub kind: K,                    // Token 类型
    pub span: Span,                 // 源代码位置
    pub text: Option<String>,       // 原始文本（可选，节省内存）
}

/// Span 支持多坐标系统（为 LSP 预留）
pub struct Span {
    pub start: Position,
    pub end: Position,
}

pub struct Position {
    pub line: usize,                // 1-based，行号（人类可读）
    pub column: usize,              // 1-based，列号（字符计数，显示用）
    pub byte_offset: usize,         // 0-based，文件字节偏移（UTF-8，文件seek用）
    pub utf16_column: usize,        // 0-based，行内UTF-16偏移（LSP Position.character）
}

**字段说明**（以 `var x = "🎉";` 中字符 `x` 的位置为例）：

| 字段 | 值 | 用途 | 说明 |
|------|-----|------|------|
| `line` | `1` | 错误显示 | 1-based行号，人类可读 |
| `column` | `5` | 错误显示 | 1-based列号，跳过`var `4个字符 |
| `byte_offset` | `4` | 文件跳转 | 0-based字节偏移，用于`seek()` |
| `utf16_column` | `4` | LSP通信 | 0-based行内偏移，对应LSP的`character`字段 |

**注意**：`column`与`utf16_column`的区别：
- `column`用于**显示**（错误信息、IDE状态栏），是1-based
- `utf16_column`用于**协议通信**（LSP），是0-based行内偏移，且计算UTF-16单元数（代理对算2个）

/// Lexer 接口（Parser 只依赖此 trait）
pub trait TokenStream<K> {
    fn next_token(&mut self) -> Option<Token<K>>;
    fn peek_token(&self, offset: usize) -> Option<&Token<K>>;
    fn current_position(&self) -> Position;
}
```

#### 5.1.3 声明式语法定义（快速语法迭代）

```rust
// src/compiler/lexer/kaubo_def.rs
// 改语法只需修改此文件

lexer_definition! {
    name: Kaubo,
    
    keywords: {
        "var" => Var,
        "if" => If,
        "else" => Else,
        "json" => Json,          // DSL 入口点
        "xml" => Xml,            // 未来扩展
    },
    
    operators: {
        "==" => EqEq,            // 自动最大匹配
        "!=" => NotEq,
        "<=" => Le,
        "+" => Plus,             // 单字符自动处理
    },
    
    literals: {
        string: { quotes: ['"', "'"], escape: true },
        integer: { base: [Decimal, Hex] },
    },
    
    comments: {
        line: "//",
        block: ("/*", "*/"),
    },
}
```

#### 5.1.4 流式与增量支持

```rust
/// 流式解析状态
pub enum StreamResult<T> {
    Token(T),           // 成功生成 Token
    Incomplete,         // 需要更多输入（流式场景）
    Error(LexError),    // 词法错误
    Eof,
}

/// 增量解析支持（IDE 实时反馈）
pub struct IncrementalLexer {
    snapshots: Vec<LexerSnapshot>,
    token_cache: LruCache<ContentHash, Vec<Token>>,
}

impl IncrementalLexer {
    /// 编辑后局部重解析
    pub fn on_edit(&mut self, edit: TextEdit) -> Vec<LexError> {
        // 1. 找到受影响的 token 范围
        // 2. 只重新扫描这段
        // 3. 如果边界 token 类型不变，停止传播
        // 4. 更新缓存
    }
}
```

#### 5.1.5 实现状态

| 阶段 | 状态 | 完成情况 |
|------|------|----------|
| Phase 0 | ✅ 完成 | 手写 Scanner 替换状态机，217 测试通过，O(n) 复杂度 |
| Phase 1 | 🚧 待开始 | SourceSpan 集成到错误系统 |
| Phase 2 | 📋 计划中 | 增量解析支持（REPL 实时反馈） |
| Phase 3 | 📋 规划中 | JSON/Vue Scanner 复用 Core 层 |

**关键变更**:
- Token 结构: `coordinate: Coordinate` → `span: SourceSpan`
- Token 文本: `value: String` → `text: Option<String>`
- Parser 访问: `token.coordinate.line` → `token.span.start.line`
- 已移除: `state_machine/` 目录、`c_lexer.rs`（旧实现）

---

### 5.2 语法分析器 (Parser)

**当前状态**: 递归下降 + Pratt 表达式解析（架构健康，保留）  
**位置**: `src/compiler/parser/`  
**日志目标**: `kaubo::parser`

#### 5.2.1 架构设计

Parser 采用经典的两层架构：

```
┌─────────────────────────────────────────────────────────────────┐
│                         Parser                                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                  Pratt Parser (表达式)                    │   │
│  │                                                          │   │
│  │   parse_expression(min_precedence)                       │   │
│  │     ├── parse_unary()                                    │   │
│  │     │     └── parse_primary()                            │   │
│  │     └── while (current.precedence > min)                 │   │
│  │           ├── consume operator                           │   │
│  │           └── parse_expression(current.precedence)       │   │
│  │                                                          │   │
│  └──────────────────────────────────────────────────────────┘   │
│                              │                                   │
│                              ▼                                   │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │               Recursive Descent (语句)                    │   │
│  │                                                          │   │
│  │   parse_statement()                                      │   │
│  │     ├── parse_var_declaration()                          │   │
│  │     ├── parse_if_statement()                             │   │
│  │     ├── parse_while_loop()                               │   │
│  │     ├── parse_for_loop()                                 │   │
│  │     └── parse_expression_statement()                     │   │
│  │                                                          │   │
│  └──────────────────────────────────────────────────────────┘   │
│                              │                                   │
│                              ▼                                   │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │              Token Stream Interface                       │   │
│  │                                                          │   │
│  │   check(kind)        - 检查当前 token                    │   │
│  │   match_token(kind)  - 匹配并消费                        │   │
│  │   expect(kind)       - 期望并消费，否则报错              │   │
│  │   consume()          - 消费当前 token                    │   │
│  │                                                          │   │
│  └──────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
```

#### 5.2.2 与 Lexer 的协作

Parser 通过 `TokenStream` trait 与 Lexer 解耦：

```rust
pub struct Parser<K> {
    lexer: Box<dyn TokenStream<K>>,
    current: Option<Token<K>>,
    peeked: Vec<Token<K>>,  // 预读缓冲区（用于 lookahead）
}

impl<K: TokenKind> Parser<K> {
    /// 消费当前 token，读取下一个
    fn consume(&mut self) {
        self.current = self.lexer.next_token();
    }
    
    /// 检查当前 token 类型
    fn check(&self, kind: K) -> bool {
        self.current.as_ref().map(|t| t.kind == kind).unwrap_or(false)
    }
    
    /// 匹配并消费
    fn match_token(&mut self, kind: K) -> bool {
        if self.check(kind) { self.consume(); true } else { false }
    }
    
    /// 期望特定 token，否则报错
    fn expect(&mut self, kind: K) -> ParseResult<()> {
        if self.match_token(kind) { 
            Ok(()) 
        } else { 
            Err(self.error(UnexpectedToken { 
                found: self.current.clone(),
                expected: kind 
            }))
        }
    }
}
```

**关键设计**：Parser 不直接依赖具体 Lexer 实现，只依赖 `TokenStream` trait。这使得：
- 可以同时支持流式 Lexer 和批量 Lexer
- 测试时可以注入 Mock Token 序列
- 未来支持增量解析时，Lexer 可以独立演进

#### 5.2.3 DSL 解析策略

对于 `json { ... }` 和未来的 DSL，Parser 采用**委托模式**：

```rust
fn parse_primary(&mut self) -> ParseResult<Expr> {
    match self.current_kind() {
        // ... 基础类型
        TokenKind::Json => self.parse_json_literal(),
        TokenKind::Xml => self.parse_xml_literal(),
        // ...
    }
}

fn parse_json_literal(&mut self) -> ParseResult<Expr> {
    self.consume(); // 消费 'json'
    self.expect(TokenKind::LeftCurlyBrace)?;
    
    // 在 { ... } 内使用 JSON 专用解析逻辑
    // 可以复用 Parser 的基础设施，但语法规则不同
    let entries = self.parse_json_entries()?;
    
    self.expect(TokenKind::RightCurlyBrace)?;
    Ok(Expr::JsonLiteral(entries))
}
```

#### 5.2.4 增量解析支持

Parser 从 Lexer 接收**增量更新**：

```rust
pub struct IncrementalParser {
    lexer: IncrementalLexer,
    ast_cache: HashMap<Range<usize>, AstNode>,
}

impl IncrementalParser {
    /// 编辑后局部重解析
    pub fn on_edit(&mut self, edit: TextEdit) -> ParseResult<Module> {
        // 1. 通知 Lexer 更新
        let token_changes = self.lexer.on_edit(edit);
        
        // 2. 找到受影响的 AST 节点
        let affected = self.find_affected_nodes(&token_changes);
        
        // 3. 重新解析受影响的语句/声明
        for node in affected {
            self.reparse_node(node)?;
        }
        
        // 4. 如果结构变化（如括号不匹配），扩大重解析范围
        Ok(self.module.clone())
    }
}
```

#### 5.2.5 关键日志点

| 级别 | 目标 | 内容 |
|------|------|------|
| `TRACE` | `kaubo::parser::token` | Token 消费详情 |
| `DEBUG` | `kaubo::parser::rule` | 规则进入/退出（span） |
| `DEBUG` | `kaubo::parser::ast` | AST 节点生成 |
| `INFO` | `kaubo::parser` | 解析开始/完成 |

### 5.3 字节码编译器 (AST → Bytecode)

**位置**: `src/runtime/compiler.rs`  
**日志目标**: `kaubo::compiler`

#### 5.3.1 职责

Compiler 负责将 Parser 生成的 AST 编译为 VM 可执行的字节码：

```
AST ──► Compiler ──► Chunk (Bytecode + Constants)
```

#### 5.3.2 关键日志点

| 级别 | 目标 | 内容 |
|------|------|------|
| `TRACE` | `kaubo::compiler::op` | 指令生成详情 |
| `DEBUG` | `kaubo::compiler::var` | 变量解析、作用域 |
| `DEBUG` | `kaubo::compiler::shape` | ShapeID 分配 |
| `INFO` | `kaubo::compiler` | 编译单元完成 |

### 5.4 虚拟机 (VM)

**位置**: `src/runtime/vm.rs`

**日志目标**: `kaubo::vm`

**关键日志点**:
- 指令执行 (`TRACE`)
- 函数调用 (`DEBUG`)
- 栈操作 (`TRACE`)

---

## 6. 内存布局

### 6.1 Value 类型 (NaN Boxing)

```
[63] Sign [62-52] Exponent(0x7FF) [51] QNAN [50-44] Tag [43-0] Payload

标签分配:
  0-7   : 特殊值 (null, true, false, SMI)
  8-23  : 内联整数 (-8 ~ +7)
  32    : 堆对象 (Boxed)
  33    : 字符串
  34    : 函数
  35    : 列表
  37    : 闭包
  38    : 协程
  42    : 模块
  43    : 原生函数
```

### 6.2 模块 ShapeID

```rust
// 编译期确定
"std" 模块:
  "print"     -> ShapeID 0
  "assert"    -> ShapeID 1
  "type"      -> ShapeID 2
  "sqrt"      -> ShapeID 4
  "PI"        -> ShapeID 9

// 运行时访问
ModuleGet 0   // O(1) 直接索引
```

---

## 7. 文件结构

```
src/
├── lib.rs              # 库入口：导出公共 API
├── main.rs             # CLI 入口：参数解析、流程调度
├── api.rs              # 【新增】高层 API (compile, run 等)
├── config.rs           # 【新增】全局配置
├── logger.rs           # 【新增】日志初始化
│
├── compiler/           # 编译器前端
│   ├── lexer/
│   │   ├── builder.rs
│   │   ├── token_kind.rs
│   │   └── logging.rs  # 【新增】Lexer 日志工具
│   └── parser/
│       ├── parser.rs
│       ├── expr.rs
│       ├── stmt.rs
│       └── logging.rs  # 【新增】Parser 日志工具
│
├── runtime/            # 运行时
│   ├── bytecode/
│   ├── stdlib/
│   ├── vm.rs
│   ├── vm/
│   │   └── logging.rs  # 【新增】VM 日志工具
│   ├── compiler.rs
│   ├── object.rs
│   └── value.rs
│
└── kit/                # 工具库

tests/
├── api_tests.rs        # 【新增】API 层测试
├── lexer_tests.rs      # 【新增】Lexer 独立测试
├── parser_tests.rs     # 【新增】Parser 独立测试
├── compiler_tests.rs   # 【新增】Compiler 独立测试
├── vm_tests.rs         # VM 执行测试
├── stdlib_tests.rs     # 标准库测试
└── common/
    └── mod.rs          # 测试共享工具
```

---

## 8. 未来功能扩展路径

基于新的 Lexer-Parser 架构，以下功能可以**逐步添加**：

### 8.1 语法高亮 (TreeSitter)

```rust
// 为 TreeSitter 提供外部 scanner
#[no_mangle]
pub extern "C" fn tree_sitter_kaubo_external_scaner_scan(...) -> bool {
    // 复用 KauboScanner
}
```

**交付物**：VS Code、Neovim、Sublime 的语法高亮插件  
**依赖**：Phase 0 完成  
**工作量**：2-3 天

### 8.2 Language Server (LSP)

```rust
pub struct KauboLanguageServer {
    lexer: IncrementalLexer,      // 增量词法分析
    parser: IncrementalParser,    // 增量语法分析
    symbols: SymbolTable,         // 符号表
    types: TypeChecker,           // 类型检查（未来）
}

impl LanguageServer for KauboLanguageServer {
    fn on_change(&mut self, changes: &[TextEdit]) {
        // 1. 增量词法分析
        let errors = self.lexer.on_edit(changes);
        
        // 2. 增量语法分析
        let ast = self.parser.incremental_parse(&self.lexer);
        
        // 3. 更新诊断信息
        self.publish_diagnostics(errors);
    }
    
    fn goto_definition(&self, pos: Position) -> Option<Location> {
        self.symbols.find_definition(pos)
    }
    
    fn completion(&self, pos: Position) -> Vec<CompletionItem> {
        self.get_completion_context(pos).suggest()
    }
}
```

**功能清单**：

| 功能 | 依赖 | 工作量 | 优先级 |
|------|------|--------|--------|
| 语法错误诊断 | Phase 1 错误系统 | 1天 | P0 |
| 实时错误提示 | Phase 2 增量解析 | 2天 | P0 |
| 跳转到定义 | Symbol Table | 3天 | P1 |
| 自动补全 | Scope Analysis | 3天 | P1 |
| 悬停提示 | Type System | 5天 | P2 |
| 重命名 | 引用分析 | 5天 | P2 |
| 代码格式化 | AST Printer | 3天 | P2 |

### 8.3 实时 REPL

```rust
pub struct KauboRepl {
    lexer: IncrementalLexer,
    parser: IncrementalParser,
    vm: Vm,
}

impl KauboRepl {
    pub fn run_line(&mut self, input: &str) -> Result<Value, ReplError> {
        match self.parser.try_parse_statement(&mut self.lexer) {
            ParseStatus::Complete(stmt) => {
                let chunk = compile(&stmt)?;
                Ok(self.vm.execute(&chunk)?)
            }
            ParseStatus::Incomplete => {
                // 提示用户继续输入（...）
                Err(ReplError::NeedMoreInput)
            }
            ParseStatus::Error(e) => Err(e.into()),
        }
    }
}
```

**效果**：
```text
kaubo> var x = 1
...    +
...    2
...    ;              // 检测到完整 statement，执行
=> 3

kaubo> var f = |x| {  // 多行 lambda 自然支持
...    return x * 2;
...    };
=> <function>
```

### 8.4 多 DSL 支持

基于统一的 Scanner 架构，新 DSL 的实现成本：

| DSL | 代码量 | 复用率 | 关键特性 |
|-----|--------|--------|----------|
| JSON | ~80行 | 80% | 复用 core scanner |
| Vue 模板 | ~200行 | 70% | 模式切换系统 |
| SQL | ~100行 | 85% | 关键字 + 字符串 |
| Markdown | ~150行 | 75% | 行级解析 |

---

## 9. 关键设计决策

### 9.1 通用决策

| 决策 | 方案 | 原因 |
|------|------|------|
| 日志框架 | `tracing` | 结构化、异步、支持 span |
| CLI 框架 | `clap` | 声明式、功能全、文档好 |
| 全局配置 | `once_cell` | 线程安全、延迟初始化 |
| 错误处理 | `thiserror` | 类型安全、自动实现 Error |
| 配置传递 | 单例模式 | 避免层层传递参数 |
| 日志级别 | 分阶段独立控制 | 便于调试特定阶段问题 |

### 9.2 Lexer 改造决策

| 决策 | 方案 | 原因 |
|------|------|------|
| 核心算法 | 手写直接扫描 | O(n) 复杂度，缓存友好，易于扩展 |
| 架构模式 | 三层架构 (Infra-Core-DSL) | 基础设施复用，DSL 快速迭代 |
| 语法定义 | 声明式宏 (`lexer_definition!`) | 改语法只需改配置，编译时优化 |
| 坐标系统 | UTF-8 + UTF-16 双坐标 | 支持 LSP 协议，零运行时开销 |
| 错误恢复 | Panic Mode | 简单有效，业界标准 |
| 流式支持 | `StreamResult<T>` | 阻塞/非阻塞统一接口 |
| 增量解析 | Token 缓存 + 范围检测 | IDE 实时反馈基础 |

### 9.3 Parser 协作决策

| 决策 | 方案 | 原因 |
|------|------|------|
| 解析算法 | 递归下降 + Pratt | 表达力强，左递归安全，易于维护 |
| 接口设计 | `TokenStream` trait | Lexer/Parser 解耦，支持多种输入源 |
| DSL 支持 | Parser 主导 + Lexer 辅助 | JSON/模板等复杂结构在 Parser 层处理 |
| 增量支持 | 基于 Lexer 增量更新 | Parser 复用 Lexer 的局部重解析能力 |

---

## 10. 实施路线图

```
Phase 0: 基础重构 (2周)
├── CharStream + Position 系统
├── KauboScanner 手写实现
├── API 兼容层
└── 性能基准测试

Phase 1: 错误系统 (1周)
├── 统一 LexError 类型
├── SourceSpan 精准定位
├── 美观错误渲染
└── 错误恢复策略

Phase 2: 增量解析 (2周)
├── IncrementalLexer
├── Token 缓存系统
├── on_edit 局部重解析
└── LSP 位置映射

Phase 3: DSL 生态 (2周)
├── JsonScanner
├── VueScanner (模式切换演示)
├── 声明式语法定义宏
└── DSL 开发文档

Phase 4+: IDE 集成 (按需)
├── TreeSitter 绑定
├── LSP Server
├── REPL 增强
└── 在线 Playground
```

---

*最后更新: 2026-02-11*  
*版本: 3.1 (Lexer 架构升级方案)*
