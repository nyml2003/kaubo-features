...
var x = 5;           // 整数
var name = "Kaubo";  // 字符串
var pi = 3.14159;    // 浮点数（注意：字面量暂不支持，需通过标准库获得）
var flag = true;     // 布尔值
var empty = null;    // 空值
```
...
| 类型 | 示例 | 说明 |
|------|------|------|
| `int` | `42`, `-10` | 32/64位整数（SMI: -2^30 ~ 2^30-1） |
| `float` | `3.14`, `-0.5` | 64位浮点数（注意：暂不支持浮点数字面量） |
| `string` | `"hello"` | 字符串 |
| `bool` | `true`, `false` | 布尔值 |
| `null` | `null` | 空值 |
| `list` | `[1, 2, 3]` | 列表/数组 |
| `json` | `json { "key": value }` | JSON 对象 |
| `function` | `\|x\| { x }` | 函数/闭包 |
...
**注意**：`and`/`or` 编译器已生成短路代码，但 VM 执行时暂未完全优化。
