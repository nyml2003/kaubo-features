...
> ⚠️ **警告：本文档描述的是目标架构，尚未实施**
>
> 当前项目仍为单 crate 架构（`src/` 目录）。本文档用于 Workspace 拆分的规划和参考。
> 
> 实际代码结构请参考 `AGENTS.md`。

本文档详细说明 Kaubo 的 Workspace 拆分方案，包括 crate 设计、迁移步骤和最佳实践。

## 背景

原始的单 crate 架构中：
- `src/bin/kaubo.rs` 包含了 CLI 特有的逻辑（参数解析、文件读取）
- `src/platform/` 虽然隔离了平台代码，但边界不够清晰
- 无法方便地支持多平台（Web/WASM）

新的 Workspace 架构明确分离：
- **核心逻辑**（`kaubo-core`）- 纯内存操作
- **执行编排**（`kaubo-api`）- 跨平台通用接口
