...
use crate::vm::core::OperatorTableEntry;
use crate::vm::core::{
...
pub fn run(vm: &mut VM) -> InterpretResult {
    use crate::vm::runtime::vm::{call, index, operators, shape, stack};
    ...
    loop {
        ...
        vm.advance_ip(1);
        let op = unsafe { std::mem::transmute::<u8, crate::vm::core::OpCode>(instruction) };
        ...
        match op {
            ...
            Call => {
                ...
                // 检查是否为内置类型方法编码
                use crate::vm::core::builtin_methods::decode_method;
                if let Some((type_tag, method_idx)) = decode_method(callee) {
                    use crate::vm::core::builtin_methods::builtin_types;
            ...
            LoadMethod => {
                ...
                // 检查是否为内置类型
                if receiver.is_list() {
                    use crate::vm::core::builtin_methods::{encode_method, builtin_types};
