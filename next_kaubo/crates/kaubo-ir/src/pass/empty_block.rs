...
mod tests {
    ...
    fn elim_preserves_const_fixture() {
        ...
        assert!(
            live.iter().any(|b| b.id == f.entry),
            "entry block must survive"
        );
    }
            ...
            functions: vec![CpsFunction {
                ...
                blocks: vec![
                    ...
                    CpsBlock {
                        ...
                        params: vec![],
                        instrs: vec![],
                        term: CpsTerminator::Branch(0, 2, vec![], 3, vec![]),
                    },
                    // blk2: then-body → Jump(3)  (to skip/merge block)
                    CpsBlock {
                        id: 2,
                        params: vec![],
                        instrs: vec![CpsInstr::LoadConst(1, 1)],
                        term: CpsTerminator::Jump(3, vec![]),
                    },
