...
/// 闭包调用回调：由运行时层提供实现
pub type ClosureCallFn = fn(vm: &mut VM, closure: *mut crate::object::ObjClosure, args: &[Value]) -> Result<Value, String>;
/// 全局闭包调用回调（运行时 init 时设置）
pub static mut CLOSURE_CALL_CB: Option<ClosureCallFn> = None;
/// 由运行时调用以注册闭包调用函数
pub fn set_closure_call_cb(f: ClosureCallFn) {
...
fn list_map(vm: &mut VM, receiver: Value, args: &[Value]) -> Result<Value, String> {
    ...
    if let Some(ptr) = receiver.as_list() {
        ...
        // 创建新列表
        let new_list = crate::object::ObjList::from_vec(results);
...
mod tests {
    ...
    /* TODO: list_push etc not yet in kaubo-ir
    #[test]
    fn test_list_push() {
        let mut vm = VM::new();
