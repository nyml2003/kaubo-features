...
mod tests {
    ...
    use kaubo_ir::{Chunk, VM, InterpretResult};
    ...
    fn compile_to_binary(_entry_path: &str, _root_dir: &str) -> Vec<u8> {
        ...
        // 创建 String Pool
        let mut string_pool = crate::binary::StringPool::new();
        ...
        for (_idx, (name, chunk)) in chunks.iter().enumerate() {
            ...
            module_entries.push(crate::binary::ModuleEntry {
                name_idx,
        ...
        // 写入 Module Table
        let mut module_table = crate::binary::ModuleTable::new();
...
"#;
        ...
        module_table.add(crate::binary::ModuleEntry {
            name_idx,
            source_path_idx: path_idx,
            chunk_offset: 0,
    ...
    fn test_binary_roundtrip_with_execution() {
        // 创建一个简单的 Chunk
        use kaubo_ir::bytecode::OpCode;
        
        let mut chunk = Chunk::new();
        chunk.write_op(OpCode::LoadConst0, 1);
        chunk.add_constant(kaubo_ir::Value::int(42));
