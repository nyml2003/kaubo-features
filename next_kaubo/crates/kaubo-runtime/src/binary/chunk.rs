...
use kaubo_ir::bytecode::{InlineCacheSlot, MethodTableEntry, OperatorTableEntry};
use kaubo_ir::chunk::Chunk;
use kaubo_ir::object::{ObjFunction, ObjShape, ObjStruct};
...
fn encode_inline_caches(buf: &mut Vec<u8>, caches: &[kaubo_ir::operators::InlineCacheEntry]) {
    ...
    match const_type {
        ...
        ConstantType::String => {
            ...
            *offset += 4;
            ...
            // 创建 ObjString
            let obj_string = kaubo_ir::object::ObjString::new(s.to_string());
        ...
        ConstantType::List => {
            ...
            *offset += 4;
            ...
            // 创建 ObjList 并转换为指针
            let list = kaubo_ir::object::ObjList::from_vec(elements);
...
fn decode_function_from_pool(
    ...
    use kaubo_ir::object::ObjFunction;
...
fn decode_inline_caches(
    bytes: &[u8],
    offset: &mut usize,
) -> Result<Vec<kaubo_ir::operators::InlineCacheEntry>, ChunkDecodeError> {
    if bytes.len() < *offset + 4 {
        return Err(ChunkDecodeError::TooShort);
    }
    ...
    *offset += 4;
    ...
    for _ in 0..len {
        ...
        *offset += 20;
        ...
        result.push(kaubo_ir::operators::InlineCacheEntry {
            left_shape,
            right_shape,
...
/* TODO: needs orchestrator pipeline
#[cfg(test)]
mod tests {
    use crate::binary::*;
use crate::vm::VmRuntime;
    ...
    fn create_test_chunk() -> Chunk {
        ...
        // 添加一些字节码
        chunk.write_op(kaubo_ir::bytecode::OpCode::LoadConst0, 1);
        chunk.write_op(kaubo_ir::bytecode::OpCode::LoadConst1, 1);
        chunk.write_op(kaubo_ir::bytecode::OpCode::Add, 1);
