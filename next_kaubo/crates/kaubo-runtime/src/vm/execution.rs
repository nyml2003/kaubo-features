...
use kaubo_ir::OperatorTableEntry;
use super::VmRuntime;
use kaubo_ir::{
...
pub fn run(vm: &mut VM) -> InterpretResult {
    use crate::vm::{call, index, operators, shape, stack};
    ...
    loop {
        ...
        vm.advance_ip(1);
        let op = unsafe { std::mem::transmute::<u8, kaubo_ir::OpCode>(instruction) };
        ...
        match op {
            ...
            // ===== 特殊值 =====
            LoadNull => push_ok!(vm, Value::NULL),
            LoadTrue => push_ok!(vm, Value::TRUE),
            LoadFalse => push_ok!(vm, Value::FALSE),
            LoadZero => push_ok!(vm, Value::smi(0)),
            LoadOne => push_ok!(vm, Value::smi(1)),
            ...
            Add => {
                ...
                if let (Some(ap), Some(bp)) = (a.as_string(), b.as_string()) {
                    ...
                    push_ok!(vm, Value::string(Box::into_raw(string_obj)));
                    continue;
                }
            ...
            Not => {
                ...
                if v.is_truthy() {
                    push_ok!(vm, Value::FALSE);
                } else {
                    push_ok!(vm, Value::TRUE);
                }
            }
            ...
            Equal => {
                ...
                let (a, b) = pop_two_ok!(vm);
                push_ok!(vm, Value::bool_from(a == b));
            }
            ...
            Greater => {
                ...
                if (a.is_int() || a.is_float()) && (b.is_int() || b.is_float()) {
                    let af = if a.is_float() { a.as_float() } else { a.as_int().map(|n| n as f64).unwrap_or(0.0) };
                    let bf = if b.is_float() { b.as_float() } else { b.as_int().map(|n| n as f64).unwrap_or(0.0) };
                    push_ok!(vm, Value::bool_from(af > bf));
                    continue;
                }
            ...
            Call => {
                ...
                // 先弹出被调用者（栈顶）
                let callee = pop_ok!(vm);
                
                // 检查是否为内置类型方法编码
                use kaubo_ir::builtin_methods::decode_method;
                if let Some((type_tag, method_idx)) = decode_method(callee) {
                    use kaubo_ir::builtin_methods::builtin_types;
            ...
            BuildJson => {
                ...
                push_ok!(vm, Value::json(json_ptr));
            }
            ...
            BuildModule => {
                ...
                push_ok!(vm, Value::module(module_ptr));
            }

            ModuleGet => {
                // 栈顶: [module]
                let module_val = pop_ok!(vm);
            ...
            BuildStruct => {
                ...
                vm.stack
                    .push(Value::struct_(Box::into_raw(struct_obj)));
            }
            ...
            LoadMethod => {
                ...
                // 栈顶是 receiver
                let receiver = peek_ok!(vm, 0);
                
                // 检查是否为内置类型
                if receiver.is_list() {
                    use kaubo_ir::builtin_methods::{encode_method, builtin_types};
                    let encoded = encode_method(builtin_types::LIST, method_idx);
                    push_ok!(vm, Value::smi(encoded));
            ...
            IndexGet => {
                // 栈顶: [index, object]
                let index_val = pop_ok!(vm);
                let obj_val = pop_ok!(vm);
            ...
            CastToBool => {
                let v = pop_ok!(vm);
                push_ok!(vm, Value::bool_from(v.is_truthy()));
            }
