...
use kaubo_web_api::token::{classify_token, utf16_range, describe_token};
...
pub fn lex(source: &str) -> String {
    ...
    let items: Vec<String> = tokens.iter()
        .filter(|t| !matches!(t.kind, kaubo_syntax::TokenKind::Whitespace | kaubo_syntax::TokenKind::Eof))
        .map(|t| {
...
pub fn compile(source: &str) -> Result<usize, JsValue> {
    ...
    kaubo_infer::infer_module(&module)
        .map_err(|e| JsValue::from_str(&e.msg))?;
...
pub fn run(_bytes: &[u8]) -> Result<String, JsValue> {
    ...
    let result = vm.execute(func_idx, reg_count)
