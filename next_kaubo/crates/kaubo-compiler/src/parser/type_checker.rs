...
use crate::parser::error::ErrorLocation;
use crate::parser::expr::{
    AsExpr, Binary, Expr, ExprKind, FunctionCall, IndexAccess, Lambda, MemberAccess, StructLiteral,
    Unary, VarRef,
};
...
impl TypeChecker {
    ...
    pub fn check_expression(&mut self, expr: &Expr) -> TypeCheckResult<Option<TypeExpr>> {
        ...
        match expr.as_ref() {
            ...
            ExprKind::As(as_expr) => {
                let _from = self.check_expression(&as_expr.expr)?;
                // Validate that target type is known
                match &as_expr.target_type {
                    TypeExpr::Named(n) if matches!(n.name.as_str(), "int" | "float" | "string" | "bool") => {
                        Ok(Some(as_expr.target_type.clone()))
                    }
                    _ => Ok(Some(TypeExpr::named("any"))),
                }
            }
    ...
    fn is_compatible(&self, from: &TypeExpr, to: &TypeExpr) -> bool {
        ...
        if matches!(from, TypeExpr::Named(n) if n.name == "null") { return true; }
        if matches!(to, TypeExpr::Named(n) if n.name == "any") { return true; }
        if let (TypeExpr::Named(from_named), TypeExpr::Named(to_named)) = (from, to) {
            if from_named.name == "int" && to_named.name == "float" { return true; }
        }
        false
    }
}
