...
use crate::lexer::core::{SourcePosition, SourceSpan, StreamResult};
...
pub trait Scanner {
    ...
    fn next_token(&mut self, stream: &mut crate::lexer::core::CharStream) -> ScanResult<Token<Self::TokenKind>>;

    /// 错误恢复：跳过到下一个安全位置
    ///
    /// 默认实现跳过到下一个空白符或已知分隔符
    fn recover_error(&mut self, stream: &mut crate::lexer::core::CharStream) -> RecoveryAction {
        // 默认实现：跳过非法字符
