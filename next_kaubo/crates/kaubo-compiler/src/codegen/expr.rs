...
use crate::parser::expr::{FunctionCall, Lambda};
use crate::parser::{Binary, Expr, ExprKind, TypeExpr};
use kaubo_ir::{
...
fn compile_function_call(compiler: &mut Compiler, call: &FunctionCall) -> Result<(), CompileError> {
    ...
    if let ExprKind::MemberAccess(member) = call.function_expr.as_ref() {
        ...
        if let Some(BuiltinType::List) = receiver_type {
            use kaubo_ir::builtin_methods::{BuiltinMethodTable, builtin_types};
            ...
            if let ExprKind::MemberAccess(member) = call.function_expr.as_ref() {
                ...
                match receiver_type {
                    Some(BuiltinType::List) => {
                        use kaubo_ir::builtin_methods::BuiltinMethodTable;
