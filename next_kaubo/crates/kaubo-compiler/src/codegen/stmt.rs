...
use crate::parser::expr::{VarRef};
use crate::parser::stmt::{ForStmt, IfStmt, WhileStmt};
...
pub fn compile_stmt(compiler: &mut Compiler, stmt: &Stmt) -> Result<(), CompileError> {
    match stmt.as_ref() {
        StmtKind::Expr(expr) => {
            super::expr::compile_expr(compiler, &expr.expression)?;
            // 表达式语句的结果丢弃
        ...
        StmtKind::VarDecl(decl) => {
            ...
            // 编译初始化表达式
            super::expr::compile_expr(compiler, &decl.initializer)?;
        ...
        StmtKind::Return(ret) => {
            if let Some(value) = &ret.value {
                super::expr::compile_expr(compiler, value)?;
        ...
        StmtKind::Print(print) => {
            super::expr::compile_expr(compiler, &print.expression)?;
...
fn compile_impl_block(
    ...
    impl_stmt: &crate::parser::stmt::ImplStmt,
) -> Result<(), CompileError> {
    ...
    for (i, method) in impl_stmt.methods.iter().enumerate() {
        ...
        if let ExprKind::Lambda(lambda) = method.lambda.as_ref() {
            ...
            for (param_name, param_type) in &lambda.params {
                ...
                } else if let Some(crate::parser::type_expr::TypeExpr::Named(named_type)) = param_type {
                    method_compiler.var_types.insert(
                        param_name.clone(),
            ...
            let function = Box::new(kaubo_ir::object::ObjFunction::new(
...
fn compile_if(compiler: &mut Compiler, if_stmt: &IfStmt) -> Result<(), CompileError> {
    // 编译 if 条件
    super::expr::compile_expr(compiler, &if_stmt.if_condition)?;
    ...
    {
        // 编译 elif 条件
        super::expr::compile_expr(compiler, elif_cond)?;
...
fn compile_import(
    compiler: &mut Compiler,
    import_stmt: &crate::parser::stmt::ImportStmt,
) -> Result<(), CompileError> {
