...
use crate::parser::{Expr, ExprKind, Module, Stmt};
use kaubo_ir::{Chunk, OpCode};
...
impl Compiler {
    ...
    /// 推断 unary 运算符的返回类型
    fn infer_unary_return_type(&self, unary: &crate::parser::expr::Unary) -> Option<VarType> {
        match unary.op {
            crate::lexer::token_kind::KauboTokenKind::Minus => {
                // 取负运算符：返回类型与操作数相同
    ...
    /// 推断函数调用的返回类型
    fn infer_call_return_type(&self, call: &crate::parser::expr::FunctionCall) -> Option<VarType> {
    ...
    /// 推断 binary 运算符的返回类型
    fn infer_binary_return_type(&self, binary: &crate::parser::expr::Binary) -> Option<VarType> {
        let left_type = self.get_expr_type(&binary.left)?;
        let right_type = self.get_expr_type(&binary.right)?;
        
        // 获取运算符名称
        let op_name = match binary.op {
            crate::lexer::token_kind::KauboTokenKind::Plus => "add",
            crate::lexer::token_kind::KauboTokenKind::Minus => "sub",
            crate::lexer::token_kind::KauboTokenKind::Asterisk => "mul",
            crate::lexer::token_kind::KauboTokenKind::Slash => "div",
            crate::lexer::token_kind::KauboTokenKind::Percent => "mod",
            // 比较运算符返回 bool，但我们不跟踪 bool 类型，返回 None
            crate::lexer::token_kind::KauboTokenKind::LessThan => return None,
            crate::lexer::token_kind::KauboTokenKind::LessThanEqual => return None,
            crate::lexer::token_kind::KauboTokenKind::GreaterThan => return None,
            crate::lexer::token_kind::KauboTokenKind::GreaterThanEqual => return None,
            crate::lexer::token_kind::KauboTokenKind::DoubleEqual => return None,
            crate::lexer::token_kind::KauboTokenKind::ExclamationEqual => return None,
            _ => return None,
        };
...
mod tests {
    ...
    use kaubo_ir::{InterpretResult, Value, VM};
    use kaubo_runtime::vm::VmRuntime;
    use kaubo_ir::object::ObjShape;
    ...
    fn compile_code(code: &str) -> Result<(Chunk, usize, HashMap<String, (u16, Vec<String>, Vec<String>)>), CompileError> {
        ...
        for stmt in &ast.statements {
            if let crate::parser::StmtKind::Struct(struct_stmt) = stmt.as_ref() {
        ...
        // 创建带日志的编译器
        let logger = kaubo_log::LogConfig::test().init().0;
    ...
    fn run_code(code: &str) -> Result<Value, String> {
        ...
        // 创建带日志的 VM
        let config = kaubo_log::LogConfig::test();
        let (logger, _ring) = config.init();
        let mut vm = VM::with_config_and_logger(kaubo_ir::VMConfig::default(), logger);
