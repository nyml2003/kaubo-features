...
| 元组 + 函数调用语义 | [04-expressions](04-expressions.md) [03-types](03-types.md) | Phase 1 ✅ |
| TypedArray (Int64Array / Float64Array) | [09-builtins](09-builtins.md) | Phase 1 ✅ |
...
| 2 | 模块系统 | L3 | ~720 | ✅ 已完成 |
| 3 | 元组 + 函数调用语义 + TypedArray | L3 | ~700 | ✅ 已完成 |
| 4 | 显式泛型 | L3 | ~1200 | 设计阶段 |
...
```
已完成 ── 语法糖 + enum/ADT + match + interface/operator + 模块系统 + 元组/TypedArray
  ▼
下一步 ── Phase 2 LSP（LspCoordinator 基于 SemanticArtifact）
  ▼
之后 ── Phase 3 prelude.kb + 编译器去硬编码
  ▼
之后 ── 泛型 + 效应系统（按需推进）
```
