...
| DAG 调度器 | `&dyn EventHandler` 不存在全局状态，未来可无缝升级为 `Arc<dyn EventHandler + Sync>` 透传 | `kaubo-driver` 重构 |
| 语义事实层 | 不影响日志/事件系统 | `kaubo-infer` + `kaubo-driver` |
| 模块系统 | 循环计数器在 `load` 时重置，每次 DAG build 新建 VM 天然隔离 | `kaubo-driver` |
