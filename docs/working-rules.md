...
本文档是仓库级协作约定，内容来自根目录 `AGENTS.md`，并作为后续架构文档的执行前提。
...
- 除非明确要求破坏兼容，否则尽量保持公共接口稳定。
- 新引入的 Rust、TypeScript 或 lint warning 必须在同一次改动里处理掉。
- 一次改动如果会碰到多层，只做能完成目标的最窄那一层。
...
- 能拆开测就拆开测：
  ...
  - 执行行为测 VM
  - 适配层和 UI glue 测 app
- 修 bug 时，优先在最窄、最有用的层补回归测试。

## 仓库卫生

- 不要提交生成产物、构建输出、测试结果或安装包目录。
- 除非任务明确要求，不要碰历史文档和归档代码。
- 如果重命名或移动某个子系统，要把相关文档和测试一起更新。
